...
QueueHandle_t playerQueue;
  ...
  void ResetChip(){
    ...
    //Player::Player(): Audio(true, I2S_DAC_CHANNEL_LEFT_EN)  {}  // only left for CYD board
  #endif
#endif
...
void Player::init() {
  ...
  setOutputPins(false);
  setOutput(config.store.dbgtouch);   // I2S or SPDIF
  delay(50);
...
void Player::loop() {
  ...
  if(xQueueReceive(playerQueue, &requestP, isRunning()?PL_QUEUE_TICKS:PL_QUEUE_TICKS_ST)){
    switch (requestP.type){
      ...
      #endif
      case PR_VUTONUS: {
        if(config.vuThreshold>5) config.vuThreshold -=5;
        break;
      }
      case PR_BURL: {
      #ifdef MQTT_ROOT_TOPIC
...
//    setOutputPins(true);
    ...
    pm.on_start_play();
#if I2S_INTERNAL
    uint32_t s32 = 0x80008000;
    size_t m_i2s_bytesWritten = 0;
    i2s_write((i2s_port_t) I2S_NUM_0, (const char*) &s32, sizeof(uint32_t), &m_i2s_bytesWritten, 100);
#endif
    setOutputPins(true);
  }else{

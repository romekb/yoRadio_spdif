...
void Audio::setOutput(bool spdf)
{
    m_spdif_output = spdf;
    if(SPDIF_OUT == 255) m_spdif_output = false;
...
size_t Audio::readAudioHeader(uint32_t bytes){
    ...
    if(m_codec == CODEC_OGG)    { m_controlCounter = 100; eofHeader = true; }
    if(!isRunning()){
...
void Audio::processLocalFile() {
    ...
    if(m_f_firstCall) {  // runs only one time per connection, prepare for start
        ...
        if(m_codec == CODEC_MP3) {                  // read ID3v1 tag from MP3 file
            ...
            if(!setFilePos(m_resumeFilePos ? m_resumeFilePos:m_audioDataStart)) audiofile.seek(m_audioDataStart);
            InBuff.resetBuffer();
        }
        return;
    }
...
*/            
    ...
    if(bytesAddedToBuffer > 0) {
        ...
        if(m_codec == CODEC_OGG) { // log_i("determine correct codec here");
            uint8_t codec = determineOggCodec(InBuff.getReadPtr(), bytesAddedToBuffer);
            if(codec == CODEC_FLAC) {
                m_codec = CODEC_FLAC;
                initializeDecoder();
                return;
...
bool Audio:: initializeDecoder(){
    switch(m_codec){
        ...
        case CODEC_VORBIS:
            ...
            AUDIO_INFO("VORBISDecoder has been initialized, free Heap: %lu bytes", ESP.getFreeHeap());
...
int Audio::sendBytes(uint8_t* data, size_t len) {
    ...
    switch(m_codec){
        ...
        case CODEC_AAC:      ret = AACDecode(data, (int32_t*)&bytesLeft, m_outBuff);    break;
        case CODEC_M4A:      ret = AACDecode(data, (int32_t*)&bytesLeft, m_outBuff);    break;
        case CODEC_FLAC:     ret = FLACDecode(data, (int32_t*)&bytesLeft, m_outBuff);   break;
        case CODEC_OPUS:     ret = OPUSDecode(data, (int32_t*)&bytesLeft, m_outBuff);   break;
        case CODEC_VORBIS:   ret = VORBISDecode(data, (int32_t*)&bytesLeft, m_outBuff); break;
...
bool Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN, int8_t MCK) {
    if(m_spdif_output) {
        m_pin_config.bck_io_num   = -1;
        m_pin_config.ws_io_num    = -1;
        m_pin_config.data_out_num = SPDIF_OUT;
...
bool Audio::setFilePos(uint32_t pos) {
    ...
    if(m_codec == CODEC_VORBIS) return false; // not impl. yet
    if(m_codec == CODEC_M4A) return false;
    if(pos >= m_file_size-4) return false;
    m_f_playing = false;
    int32_t newpos = pos;
    if(m_codec == CODEC_MP3)  {MP3Decoder_ClearBuffer(); newpos = mp3_correctResumeFilePos(pos);}
    if(m_codec == CODEC_WAV)  {while((newpos % 4) != 0) newpos++; audiofile.seek(newpos);} // must be divisible by four
    if(m_codec == CODEC_FLAC) {FLACDecoderReset(); newpos = flac_correctResumeFilePos(pos);}
    InBuff.resetBuffer();
//    if(pos < m_audioDataStart) pos = m_audioDataStart; // issue #96
//    if(pos >= m_file_size) return false;
    if(newpos>0 && m_avr_bitrate) m_audioCurrentTime = ((newpos-m_audioDataStart) / m_avr_bitrate) * 8; // #96
//    uint32_t sk = audiofile.seek(pos);
    return newpos>0 ? true:false;
}
//---------------------------------------------------------------------------------------------------------------------
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    const float FcLS   =  120;  // Frequency LowShelf[Hz] //500
    const float FcPKEQ = 2000;  // Frequency PeakEQ[Hz]

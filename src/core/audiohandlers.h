...
void audio_info(const char *info) {
  ...
  if (strstr(info, "format is opus") != NULL) { config.setBitrateFormat(BF_OPUS); display.putRequest(DBITRATE); }
  if (strstr(info, "format is vorbis") != NULL) { config.setBitrateFormat(BF_VORB); display.putRequest(DBITRATE); }

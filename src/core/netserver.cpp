...
const char *getFormat(BitrateFormat _format) {
  switch (_format) {
    ...
    case BF_OPUS: return "OPU";
    case BF_VORB: return "VOR";
    ...
    switch (request.type) {
      ...
      case GETACTIVE: {
          ...
          if (network.status == CONNECTED) {
            ...
            if (TS_MODEL!=TS_MODEL_UNDEFINED || SPDIF_OUT!=255 || dbgact) APPEND_GROUP("group_touch");
      ...
      case SDPOS:         { int32_t filepos = player.getFilePos()-player.inBufferFilled(); if(filepos<0) filepos = 0;
                          sprintf (wsBuf, "{\"sdpos\": %lu,\"sdend\": %lu,\"sdtpos\": %lu,\"sdtend\": %lu}", 
                                  filepos, 
                                  player.getFileSize(), 
